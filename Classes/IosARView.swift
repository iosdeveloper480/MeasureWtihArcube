...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    let initalPositionManagerChannel: FlutterMethodChannel

    var isCubePlaced = false
    var isPointerPlaced = false
    ...
    //TODO: Plane or something else in ARKIT, Check all vbariable types
    private var firstPlane: Plane? = nil
     private var pointerRotation:FlutterStandardTypedData(float32: Data)?=nil
    private var pointerTranslation:FlutterStandardTypedData(float32: Data)? = nil

    private var nodeMap =[String,Any]()
    private var cubeNode:Node? = nil
    ...
    init(
        ...
        self.initalPositionManagerChannel.setMethodCallHandler(self.initialPositionMethodCall)
    ...
    func initialPositionMethodCall(_ call :FlutterMethodCall, _ result: @escaping FlutterResult) {
        switch call.method{
            case "position-x":
    ...
    func renderer(_ renderer: SCNSceneRenderer, didUpdate node: SCNNode, for anchor: ARAnchor) {
        let frame = self.sceneView.arFrame!

        let var3 = frame.getUpdatedTrackables(Plane.class).iterator()
        ...
        while(var3.hasNext()) {
            ...
            if (firstPlane?.trackingState == TrackingState.TRACKING ) {
                ...
                pointerTranslation = frame.camera.pose.compose(Pose.makeTranslation(0, 0, -2)).translation
                pointerTranslation = frame.camera.pose.compose(Pose.makeTranslation(0, 0, -2)).translation

                 let pose = Pose(pointerTranslation,pointerRotation)

                /// Creating the pointer

                if (isPointerPlaced){
                    cubeNode?.worldPosition = Vector3(pointerTranslation?.get(0)!,pointerTranslation?.get(1)!,pointerTranslation?.get(2)!)
                    cubeNode?.worldRotation = Quaternion(Vector3(pointerRotation?.get(0)!,pointerRotation?.get(1)!,pointerRotation?.get(2)!))
                } else {
                    removeSquareAnchor()
                    isPointerPlaced = true
                    let loader: FlutterLoader = FlutterInjector.instance().flutterLoader()
                    let key: String = loader.getLookupKeyForAsset(nodeMap!["uri"] as String)

                    modelBuilder.makeNodeFromGltf(viewContext, nodeMap!["name"] as String, key, nodeMap!["transformation"] as ArrayList<Double>)
                            .thenAccept { node ->
                                isPointerPlaced = true
    ...
    func removeSquareAnchor(){
        var anchorNode = sceneView.findByName("sqaure") as AnchorNode?
        if(anchorNode != nil){
                            ...
                            switch anchorType{
                                case 0: //PlaneAnchor
                                    if let anchor = self.anchorCollection[anchorName]{
                                        ...
                                        node.worldPosition = Vector3(pointerTranslation?.get(0)!,pointerTranslation?.get(1)!,pointerTranslation?.get(2)!)
                                        node.worldRotation = Quaternion(Vector3(pointerRotation?.get(0)!,pointerRotation?.get(1)!,pointerRotation?.get(2)!))
